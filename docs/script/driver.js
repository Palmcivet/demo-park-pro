...
// 12个矩形
elements.push(
	{ width: 40, height: 26, top: 50, left: 42, index: 1 },
...
function choice(event) {
	var x = event.pageX - elemLeft - 4,
		y = event.pageY - 156;

	elements.forEach(function (ele) {
		if (
			y > ele.top &&
			y < ele.top + ele.height &&
			x > ele.left &&
			x < ele.left + ele.width
		) {
			...
			function savenode() {
				...
				localStorage.setItem(
					"node" + nodeindex,
					JSON.stringify(start_node[nodeindex])
				);
			}
			...
			function initnode() {
				...
				if (save_node == null) {
					start_node[ele.index] = new Node();
					start_node[ele.index].rect = new Rect(
						ele.left,
						ele.top,
						ele.width,
						ele.height
